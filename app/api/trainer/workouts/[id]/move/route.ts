...
import { moveWorkoutInputSchema } from "@/modules/workouts/schedule-schema";
import {
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const input = await parseJsonBody(request, moveWorkoutInputSchema);
    const trainerProfile = await prisma.trainerProfile.findUniqueOrThrow({
      where: { userId: identity.userId },
    });

    const workout = await moveWorkout(id, input, {
