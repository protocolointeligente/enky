...
import { applyWorkoutTemplateInputSchema } from "@/modules/templates/template-schema";
import { applyTemplate } from "@/modules/templates/template-service";
import {
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const input = await parseJsonBody(request, applyWorkoutTemplateInputSchema);
    const trainerProfile = await prisma.trainerProfile.findUniqueOrThrow({
      where: { userId: identity.userId },
    });

    const workout = await applyTemplate(id, input, {
