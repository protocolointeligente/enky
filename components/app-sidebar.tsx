...
// Shell com navegação persistente para a área do treinador.
// Desktop: sidebar fixa, recolhível (preferência em localStorage).
// Mobile: topbar compacta com scroll horizontal.
//
// Link ativo: borda-left laranja + fundo surface-2 (identidade Command Center).
// Grupos de navegação separados por seção visual.

const STORAGE_KEY = "enky:sidebar-collapsed";

// ── Ícones inline (Lucide-style 20×20) ──────────────────────────────────────
const NAV_ICONS: Record<string, React.ReactNode> = {
  ...
  periodizacao: (
    <path d="M12 3 3 8l9 5 9-5-9-5zM3 13l9 5 9-5M3 17l9 5 9-5" />
  ),
  exercicios: (
...
export function AppSidebar({
  ...
  return (
    <div className="lg:flex">
      ...
      <aside
        className={`sticky top-0 hidden h-screen shrink-0 flex-col border-r border-line bg-petrol transition-[width] duration-200 lg:flex ${
      ...
      <div className="min-w-0 flex-1">
        ...
        <header className="sticky top-0 z-30 flex items-center justify-between gap-3 border-b border-line bg-petrol/95 px-4 py-3 backdrop-blur lg:hidden">
          ...
          <nav
            ...
            className="flex items-center gap-0.5 overflow-x-auto whitespace-nowrap scrollbar-none"
          >
            {links.map((link) => {
              const active = isActive(link.href);
              return (
          ...
          <div className="flex shrink-0 items-center gap-2">
            ...
            <button
              ...
              onClick={handleLogout}
              disabled={loggingOut}
              className="rounded-lg border border-line px-2.5 py-1.5 text-xs font-medium text-muted transition-colors hover:border-line-strong hover:text-ink disabled:opacity-50"
            >
