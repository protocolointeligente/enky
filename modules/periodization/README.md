...
**Fase 04 — plano estratégico rico.** A criação passou a um modal sobreposto (`components/periodization-create-modal.tsx`) com dropdowns e **parâmetros por modalidade** (VDOT/pace na corrida, FTP/TSS no ciclismo, CSS na natação, séries/RIR/tonelagem/grupos na musculação, padrão de movimento no funcional). `Periodization` ganhou (migration aditiva `20260718110000`): `modality`, `targetEvent`, `level`, `loadControlMethod`, `mainUnit`, `totalVolume`, `mesocycleCount`, `microcycleCount`, `recoveryStrategy`, `difficultyDistribution`, `autoGenerate`, `isDraft`, `notes` e `parameters` (JSON validado por `periodizationParametersSchema`, nunca lido cru). Um plano pode ser salvo como **rascunho** (`isDraft`) incompleto; um plano completo exige modalidade e objetivo (validado no Zod, `tests/unit/.../periodization-schema.test.ts`). Grupo de atletas segue single-atleta por plano (ADR-001; multiusuário/grupo é fase posterior).
